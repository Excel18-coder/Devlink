...
router.get("/analytics", requireAuth, requireRole("admin"), asyncHandler(async (_req, res) => {
    const now = new Date();
    const sixMonthsAgo = new Date(now.getFullYear(), now.getMonth() - 5, 1);
    const [totalUsers, totalDevelopers, totalEmployers, openJobs, activeContracts, userTrend, recentUsers, recentJobs, contractsByStatusAgg, contractsTrend, totalEscrowAgg, totalPaidOutAgg, jobsByStatusAgg, jobsByTypeAgg, usersByRoleAgg, totalApplications, showcaseCount, commissionDoc] = await Promise.all([
        User.countDocuments(),
    ...
    const userTrendMap = new Map(userTrend.map((r) => [`${r._id.year}-${r._id.month}`, r.count]));
    const userGrowth = months.map((m) => ({ month: m.label, users: userTrendMap.get(`${m.year}-${m.month}`) ?? 0 }));
    const contractsTrendMap = new Map(contractsTrend.map((r) => [`${r._id.year}-${r._id.month}`, { count: r.count, value: r.value }]));
    ...
    const contractStatusMap = new Map(contractsByStatusAgg.map((r) => [r._id, r.count]));
    const jobStatusMap = new Map(jobsByStatusAgg.map((r) => [r._id, r.count]));
    const jobTypeMap = new Map(jobsByTypeAgg.map((r) => [r._id, r.count]));
    const roleMap = new Map(usersByRoleAgg.map((r) => [r._id, r.count]));
    const totalPaidOutVal = (totalPaidOutAgg[0]?.total ?? 0);
    ...
    return res.json({
        ...
        totalEscrow: (totalEscrowAgg[0]?.total ?? 0),
...
router.get("/analytics/stream", requireAuth, requireRole("admin"), asyncHandler(async (_req, res) => {
...
router.get("/users", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
    ...
    if (search)
        filter.$or = [{ email: new RegExp(escapeRegex(String(search)), "i") }, { fullName: new RegExp(escapeRegex(String(search)), "i") }];
...
router.patch("/users/:id/status", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
...
router.patch("/users/:id/role", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
    const { role } = req.body;
    if (!["developer", "employer", "admin"].includes(role))
...
router.patch("/jobs/:id/status", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
    const { status } = req.body;
...
router.delete("/jobs/:id", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
...
router.patch("/config", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
    ...
    if (key === "commission_pct") {
        ...
        if (isNaN(num) || num < 0 || num > 100)
            return res.status(400).json({ message: "commission_pct must be a number between 0 and 100" });
    }
    if (key === "max_file_size_mb") {
        const num = Number(value);
        if (isNaN(num) || num <= 0)
            return res.status(400).json({ message: "max_file_size_mb must be a positive number" });
    }
...
router.patch("/config/bulk", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
    ...
    for (const [key, value] of Object.entries(updates)) {
        ...
        if (key === "commission_pct") {
            const num = Number(value);
            if (isNaN(num) || num < 0 || num > 100) {
                errors.push("commission_pct must be a number between 0 and 100");
                continue;
            }
        }
        if (key === "max_file_size_mb") {
...
router.delete("/config/:key", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
    const { key } = req.params;
    const PROTECTED = ["commission_pct", "maintenance_mode", "max_file_size_mb"];
    if (PROTECTED.includes(key))
        return res.status(400).json({ message: `Config key "${key}" is protected and cannot be deleted` });
...
router.post("/disputes/:id/resolve", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
    const { resolution } = req.body;
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.status !== "disputed")
        return res.status(400).json({ message: "Contract not in dispute" });
...
// ─── Create admin user ────────────────────────────────────────────────────────────
router.post("/create-admin", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
...
router.get("/employers", requireAuth, requireRole("admin"), asyncHandler(async (_req, res) => {
    ...
    const jobCountMap = new Map(jobCounts.map((r) => [r._id.toString(), r.count]));
    const contractCountMap = new Map(contractCounts.map((r) => [r._id.toString(), r.count]));
    return res.json(employers.map((e) => {
...
router.get("/news", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
...
router.post("/news", requireAuth, requireRole("admin"), upload.single("image"), validate(createNewsSchema), asyncHandler(async (req, res) => {
    const { title, body, excerpt, category, imageUrl, status } = req.body;
...
router.patch("/news/:id", requireAuth, requireRole("admin"), validateObjectIds, upload.single("image"), validate(updateNewsSchema), asyncHandler(async (req, res) => {
...
router.delete("/news/:id", requireAuth, requireRole("admin"), validateObjectIds, asyncHandler(async (req, res) => {
