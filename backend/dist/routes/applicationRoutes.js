...
// ─── Apply for a job ──────────────────────────────────────────────────────────
router.post("/:jobId", requireAuth, requireRole("developer"), validate(createApplicationSchema), asyncHandler(async (req, res) => {
    ...
    if (job.status === "closed")
        return res.status(400).json({ message: "This position has been filled and is no longer accepting applications" });
    if (job.status === "paused")
        return res.status(400).json({ message: "This job is currently paused and not accepting applications" });
...
router.get("/job/:jobId", requireAuth, requireRole("employer", "admin"), asyncHandler(async (req, res) => {
...
router.patch("/:id/status", requireAuth, requireRole("employer"), asyncHandler(async (req, res) => {
    ...
    if (status === "accepted") {
        ...
        await Application.updateMany({ jobId: app.jobId, _id: { $ne: app._id }, status: { $in: ["submitted", "shortlisted"] } }, { status: "rejected" });
    }
...
router.get("/employer/recent", requireAuth, requireRole("employer"), asyncHandler(async (req, res) => {
