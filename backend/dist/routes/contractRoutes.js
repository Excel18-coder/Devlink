...
// ─── Create contract ────────────────────────────────────────────────────────
router.post("/", requireAuth, requireRole("employer"), validate(createContractSchema), asyncHandler(async (req, res) => {
    const { jobId, developerId, milestones } = req.body;
    const parsedMilestones = (milestones ?? []).map((ms) => ({
        title: ms.title,
        amount: ms.amount,
        dueDate: ms.dueDate ? new Date(ms.dueDate) : undefined
    }));
    const totalAmount = parsedMilestones.reduce((sum, ms) => sum + ms.amount, 0);
...
router.post("/:id/payment-details", requireAuth, requireRole("developer"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.developerId.toString() !== req.user.id)
...
router.post("/:id/milestones/:msId/submit", requireAuth, requireRole("developer"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.developerId.toString() !== req.user.id)
...
router.post("/:id/milestones/:msId/release", requireAuth, requireRole("employer"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.employerId.toString() !== req.user.id)
        return res.status(403).json({ message: "Forbidden" });
    const milestone = contract.milestones.id(req.params.msId);
    if (!milestone)
        return res.status(404).json({ message: "Milestone not found" });
    if (milestone.status !== "submitted")
        return res.status(400).json({ message: "Milestone must be submitted before it can be released" });
...
router.post("/:id/milestones/:msId/deliver", requireAuth, requireRole("developer"), uploadSubmission.single("deliveryFile"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.developerId.toString() !== req.user.id)
    ...
    if (milestone.status !== "released")
        return res.status(400).json({ message: "Milestone must be released (payment confirmed) before final delivery" });
    ...
    if (finalFileUrl)
        milestone.finalFileUrl = finalFileUrl;
    await contract.save();
    await createAuditLog({
        actorId: req.user.id,
...
router.post("/:id/milestones", requireAuth, requireRole("employer"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.employerId.toString() !== req.user.id)
    ...
    if (contract.status !== "active")
        return res.status(400).json({ message: "Can only add milestones to an active contract" });
...
router.post("/:id/complete", requireAuth, requireRole("employer"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.employerId.toString() !== req.user.id)
    ...
    if (!allDelivered)
        return res.status(400).json({ message: "All milestones must be delivered before completing the contract" });
...
router.post("/:id/terminate", requireAuth, requireRole("employer"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.employerId.toString() !== req.user.id)
...
router.patch("/:id/milestones/:msId", requireAuth, requireRole("employer"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.employerId.toString() !== req.user.id)
    ...
    if (!milestone)
        return res.status(404).json({ message: "Milestone not found" });
    if (milestone.status !== "pending")
        return res.status(400).json({ message: "Only pending milestones can be edited" });
    const { title, amount, dueDate } = req.body;
...
router.delete("/:id/milestones/:msId", requireAuth, requireRole("employer"), asyncHandler(async (req, res) => {
    const contract = await Contract.findById(req.params.id);
    if (!contract)
        return res.status(404).json({ message: "Contract not found" });
    if (contract.employerId.toString() !== req.user.id)
    ...
    if (!milestone)
        return res.status(404).json({ message: "Milestone not found" });
    if (milestone.status !== "pending")
        return res.status(400).json({ message: "Only pending milestones can be deleted" });
