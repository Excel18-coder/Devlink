...
router.get("/:id", asyncHandler(async (req, res) => {
  ...
  if (!dev || !user) return res.status(404).json({ message: "Developer not found" });
  return res.json({
...
router.patch("/me", requireAuth, requireRole("developer"), validate(updateDeveloperSchema), asyncHandler<AuthRequest>(async (req, res) => {
...
// ─── Upload resume ────────────────────────────────────────────────────────────
router.post("/me/resume", requireAuth, requireRole("developer"), upload.single("resume"), asyncHandler<AuthRequest>(async (req, res) => {
...
// ─── Upload avatar ────────────────────────────────────────────────────────────
router.post("/me/avatar", requireAuth, requireRole("developer"), upload.single("avatar"), asyncHandler<AuthRequest>(async (req, res) => {
  if (!req.file) return res.status(400).json({ message: "No file provided" });
